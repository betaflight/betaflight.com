...
const Search = (props) => {
  ...
  const getSearchDoc = () => (process.env.NODE_ENV === 'production' ? fetch(`${assetUrl}${pluginData.fileNames.searchDoc}`).then((content) => content.json()) : Promise.resolve({}));

  const getLunrIndex = () => (process.env.NODE_ENV === 'production' ? fetch(`${assetUrl}${pluginData.fileNames.lunrIndex}`).then((content) => content.json()) : Promise.resolve([]));

  const loadAlgolia = () => {
    if (!initialized.current) {
      Promise.all([getSearchDoc(), getLunrIndex(), import('./DocSearch'), import('./algolia.css')]).then(([searchDocFile, searchIndex, { default: DocSearch }]) => {
  ...
  return (
    <div className="navbar__search" key="search-box">
      <input
        ...
        aria-label="Search"
        className={clsx('navbar__search-input', { 'search-bar-expanded': props.isSearchBarExpanded }, { 'search-bar': !props.isSearchBarExpanded })}
        onClick={loadAlgolia}
