...
spin away from the camera "props out". Pilots use "props out" to prevent debris from being pushed into the camera at 
the cost of pushing it into the body of the quad instead
...
BLHELI-S, BLHELI32, BlueJay or AM32 ESCs should all use Dshot.
...
output connections the FC will also listen for return data from the ESC on the same wire. By default includes only RPM 
data but can be expanded with the `dshot_edt` CLI command to also enable ESC voltage and temperature data via dshot 
telemetry (requires newer BlueJay/AM32/BLHELI32 ESC firmware)

* **Motor Poles** - Number of permanent magnets stuck inside the motor bell. Larger motors such as 2207 or 2306 have 14
...
idle without a delay and without loosing control. Generally this speed is increased to improve smoothness when coming 
