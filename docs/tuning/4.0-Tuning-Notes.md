...
- [RPM controlled multiple dynamic notch filtering over bi-directional DShot](/docs/wiki/archive/Bidirectional-DSHOT-and-RPM-Filter), to markedly improve motor noise suppression, reducing filter delay time
- [Launch Control](/docs/wiki/archive/Launch-Control), allows a pilot to hold a specified angle precisely and accurately for the perfect race launch
- [D_min](/docs/wiki/archive/DMIN.md), which dynamically adjusts the PID D parameter according to stick movement
...
- [Integrated yaw](/docs/wiki/archive/Integrated-Yaw), an experimental option which mathematically integrates the yaw PID values, potentially simplifying yaw tuning
...
### The following notes apply ONLY to un-integrated yaw configuration! If you're flying [integrated yaw](/docs/wiki/archive/Integrated-Yaw), the tuning process is completely different!
