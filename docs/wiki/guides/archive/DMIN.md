...
:::caution Archived — Betaflight 2025.12 and later

The `d_min_*` CLI variables described in this document no longer exist in current firmware. The concept was renamed and restructured:

- **`d_min_roll` / `d_min_pitch` / `d_min_yaw`** → **`d_roll` / `d_pitch` / `d_yaw`** (base D, active in calm flight)
- **`d_max_roll` / `d_max_pitch` / `d_max_yaw`** (peak D, active during fast moves and propwash)
