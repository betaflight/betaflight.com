...
2. **`adrc_wo`** — raise until chatter appears in hover (the observer starting to track gyro noise), then back off ~20%. **Arm props-off first when trying a higher `wo`** — a self-induced ground oscillation at high `wo` can fool the gyro-only liftoff path into opening the gate while the craft is still grounded (see [Testing Notes](#testing-notes-read-before-flying)). This step is exactly the path into that condition, so watch and listen closely as you raise `wo`.
...
- **`pid_at_min_throttle = OFF`** disables the controller entirely below the arming throttle threshold — sticks/tilts produce zero motor response until throttle is raised. The default `ON` usually provides additional ground protection under ADRC via the liftoff gate — but that protection isn't absolute; see the false-liftoff caution below. Only turn it off if you specifically want the "fully asleep at idle" behavior and understand the trade-off.
...
- **If the motors are audibly oscillating at idle right after arming, disarm immediately.** A high enough `adrc_wo` can make the craft self-oscillate on the ground at idle; the liftoff gate's gyro-only path can misread that oscillation as a real liftoff and open **while the craft is still grounded**, driving the motors toward saturation. Arm props-off first when trying a higher `wo`, and watch/listen closely as you raise it. See [Known limitations](#known-limitations) for the current status of this failure mode.
...
Open, not-yet-root-caused items — including the high-`wo` ground self-oscillation / false-liftoff-detection failure mode described in [Testing Notes](#testing-notes-read-before-flying), the b0 throttle-curve law itself, and default-tune conservatism across typical 5" builds — are tracked with their current status and decision options in the remediation tracker's [open items](https://github.com/danusha2345/ADRC-betaflight/blob/master/docs/ADRC_REMEDIATION_TRACKER.md#open-items) rather than duplicated here.
