...
:::info
**Not yet in an official Betaflight release.** This page documents an open pull request, [betaflight/betaflight#15400](https://github.com/betaflight/betaflight/pull/15400) — `pid_type = ADRC` does not exist in any stock Betaflight build (Configurator releases, official `master`) until that PR merges; `set pid_type = ADRC` will simply error out on a normal build — the CLI rejects `pid_type` as an unknown setting name, since the variable itself doesn't exist there. To try it now, flash one of the PR's [prebuilt hex releases](https://github.com/danusha2345/ADRC-betaflight/releases) — use the `b4` tag, which matches the tunable set documented on this page exactly. The newer `b5` build adds a fork-side `adrc_b0_law` selector (`QUADRATIC`/`SQRT`/`LINEAR`/`FIXED`) that isn't in this branch or in the [CLI Reference](#cli-reference) below. `b5` is also persisted-group version **0, not 16** — the PG version field is only 4 bits (`pg.h`'s `PGR_PGN_VERSION_MASK = 0xf000`), and 15 was already the maximum, so it wraps — which means **flashing `b5` over a `b2`–`b4` install, or over a PR-head build, resets your PID profiles to defaults.** Run `diff all` first if you want to keep your existing tune.
:::
...
| `adrc_sigma_decay`                 | 3                  | 0–100             | z3 leaky-decay rate ×0.1 (0 = pure integrator) while airborne — default is 0.3/s, τ ≈ 3 s                                                                                                                    |
...
| `adrc_gated_z3_decay`              | 200 (=20.0)        | 0–2000            | z3 decay rate ×0.1 while the gate is closed (grounded) — never slower than `adrc_sigma_decay` (floored up to match if set lower), and never slower than an absolute 1.0/s floor regardless of either setting |
| `adrc_b0_scale_max`                | 3                  | 1–50              | Ceiling on the throttle-scaled `b0` multiplier; CLI range allows raising it for experiments                                                                                                                  |
...
2. **`adrc_wo`** — raise until chatter appears in hover (the observer starting to track gyro noise), then back off ~20%. **Arm props-off first when trying a higher `wo`.** At `wo = 150`, testing has found the craft can self-oscillate on the ground at idle (~28.5 Hz) — the gyro-only liftoff path (`adrc_liftoff_gyro_dps = 20` sustained for `adrc_liftoff_hold_ms = 25`) reads that oscillation as liftoff within 0.1–0.4 s of arming and opens the gate **with the craft still on the ground at 0% stick throttle**, driving the motors to saturation (tracked as ADRC-026 in the [remediation tracker](https://github.com/danusha2345/ADRC-betaflight/blob/master/docs/ADRC_REMEDIATION_TRACKER.md)). This step is exactly the path into that condition, so watch and listen closely as you raise `wo`.
...
- **If the motors are audibly oscillating at idle right after arming, disarm immediately.** At high `adrc_wo` a self-induced ground oscillation can trip the liftoff detector's gyro path, after which the observer spools the motors up with your throttle still down (ADRC-026 — see the [tuning procedure](#tuning-procedure) note above). Arm props-off first when trying a higher `wo`.
...
Open, not-yet-root-caused items — including the b0 throttle-curve law itself and default-tune conservatism across typical 5" builds — are tracked with their current status and decision options in the remediation tracker's [open items](https://github.com/danusha2345/ADRC-betaflight/blob/master/docs/ADRC_REMEDIATION_TRACKER.md#open-items) rather than duplicated here.
