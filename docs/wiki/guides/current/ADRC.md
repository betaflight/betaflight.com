...
- **Throttle-scaled b0** — motor authority roughly scales with throttle², so `b0` is scaled by `(throttle / adrc_hover_throttle)²` above hover, clamped to `adrc_b0_scale_max` (default 9×), keeping the plant model calibrated away from hover.
...
| `adrc_liftoff_idle_throttle`       | 5                  | 0–100             | Throttle % the craft must drop below before the gate can re-arm. Keep below both `adrc_liftoff_throttle` and your actual hover throttle |
| `adrc_liftoff_idle_hold_ms`        | 500                | 0–10000           | How long idle-throttle-and-stillness must sustain to re-arm                                                                             |
...
| `adrc_b0_scale_max`                | 9                  | 1–50              | Ceiling on the throttle-scaled `b0` multiplier                                                                                          |
