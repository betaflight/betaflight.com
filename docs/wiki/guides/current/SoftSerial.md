...
For simple position data, if the GPS module has been configured, in uCenter or pyGpsClient, to power up at a low baud rate, eg 19200 baud, or 9600 baud, and to send position data values alone at 1Hz, then Betaflight only needs a single Rx pin to receive that data. This can be a softserial pin. In the GPS tab, auto-configuration should be disabled, and the baud rate for the softserial port must be set to match the GPS's power up baud rate. Test first with a real Rx pin and be absolutely certain you are not sending unwanted data, eg satellite position information. You should not expect to see a satellite list in Configurator, only Latitude and Longitude. If this works on a real Rx pin, then test it on on a softserial pin.

When the GPS is intended for GPS Rescue purposes, Betaflight requires bi-directional communication with the GPS module during the initialisation phase, so that we can configure the data rate, request only the data we need, and block any other junk that we don't need. To configure the GPS requires both Rx and Tx lines. Using Softserial for both Rx and Tx, rather than just one wire, is problematic, and the GPS may not get configured properly. Since the maximum baud rate is 19200, the GPS data interval cannot be more than 2Hz. This will result in very jerky or un-reliable GPS Rescues and may result in rescue failure. Hence we strongly discourage the use of SoftSerial for GPS Rescue.

### Softserial with HD OSD (displayport) over MSP

This is not recommended, though it _may_ work. Typically a bidirectional stream is required, requiring configuration of both a SoftserialTx and an Softserial Rx pin. That doubles the delay problems associated with Softserial compared to single-wire softserial applications.

It may be possible to use Softserial to send data over MSP to an HD OSD device. A single SoftserialTx pin would need to be defined. The user should first check with a real UART using a single Tx pin, to make sure it works. The OSD configuration should be kept simple.
