...
- [QMC5883L](https://www.qstcorp.com/upload/pdf/202512/13-52-04%20QMC5883L%20Datasheet%20Rev.%20B.pdf)[^8]. The QMC5883L is provided on the common, and cheap, GY-271 module, and many GPS units. It provides a 200Hz data update rate, 8x sample averaging and 3000 LSB/Gauss sensitivity. Standard axis orientation. Default i2c address 13. We recommend using this mag if it is an option for your build, because its performance has been validated during testing - we know it works well. Discontinued in 2025 and replaced by the QMC5883P, see below.
- [QMC5883P](https://www.qstcorp.com/upload/pdf/202512/2C939E5AA0704285BC3BE71132B8629B.pdf)[^13]. Successor of the QMC5883L above. It features some upgraded specifications, such as increased range (up to 30 gauss, not relevant for the Earth's magnetic field) and higher sensitivity. Just as the QMC5883L it has built-in temperature compensation, up to a 200 Hz data output rate and up to 8x oversampling. Compared to the QMC5883L, the QMC5883P adds a downsampling filter for better noise performance.
- [IST8310](https://isentek.com/userfiles/files/IST8310Datasheet_3DMagneticSensors.pdf)[^9] Note that this Mag has a highly unusual axis orientation. Y is to the _right_ when X is forward and Z is up. This Mag will _always_ require a custom axis orientation in the CLI. Data update rate is 160Hz with 16x sample averaging and 330 LSB/Gauss sensitivity. Also note that the IST8310 can have any of four i2c addresses, depending on how the manufacturer of the FC wires it up. Betaflight's driver will connect automatically if the address is configured to be 14. If it does not connect automatically, try set `mag_i2c_address` to either 12, 13 or 15, one of those values should work.
- [STM's LIS3MDL](https://www.st.com/resource/en/datasheet/lis3mdl.pdf)[^10] This mag is integral to a combined Gyro, Acc and Mag '9 axis' chip from STM. Standard axis orientation. Obsolete, its replacement part is the LIS2MDL below.
- [LIS2MDL](https://www.st.com/resource/en/datasheet/lis2mdl.pdf)[^12], another magnetometer from STM. Note that the Betaflight driver reverses the X-axis to accommodate common sensor axis orientation and to match the LIS3MDL orientation from above. So if you would normally set the orientation to 180° clockwise based on the chip marker itself, you'd need to use 0° clockwise. Or vice versa. Same goes for the orthogonal orientation: 90° clockwise based on the chip marker would become 270° clockwise and vice versa.
...
[^9]: [IST8310 datasheet](https://www.isentek.com/api/download?url=https%3A%2F%2Fassets.isentek.com%2Fpublic%2Fdocuments%2Fdatasheet%2FHFH0n8I7hQ-IST8310_Datasheet_V1.8_Web.pdf&filename=IST8310_Datasheet_V1.8_Web.pdf&action=view)
...
[^13]: [QMC5883P datasheet](https://www.qstcorp.com/upload/pdf/202512/2C939E5AA0704285BC3BE71132B8629B.pdf)
