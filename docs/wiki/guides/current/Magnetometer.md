...
- [STM's LIS3MDL](https://www.st.com/resource/en/datasheet/lis3mdl.pdf)[^10] This mag is integral to a combined Gyro, Acc and Mag '9 axis' chip from STM. Standard axis orientation. Obsolete, it's replacement part is the LIS2MDL below.
- [LIS2MDL](https://www.st.com/resource/en/datasheet/lis2mdl.pdf)[^12], another magnetometer from STM. Note that the Betaflight driver reverses the X-axis to accomodate for common sensor axis orientation and to match the LIS3MDL orientation from above. So if you would normally set the orientation to 180° clockwise based on the chip marker itself, you'd need to use 0° clockwise. Or vica versa. Same goes for the orthogonal orientation: 90° clockwise based on the chip marker would become 270° clockwise and vica versa.
