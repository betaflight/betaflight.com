...
- Make sure that you are plugging the USB cable into the flight controller. A DJI Air Unit is a video transmitter.  
  Neither should you try to connect your radio to the configurator. Betaflight Configurator is not meant be used to
  set up your radio, use your radio's software for that (OpenTX companion, EdgeTX Buddy, etc.)
...
manufacturer, and flashing it will bring you back to a complete default state. Even if you are building your own
...
the UART that your VTX is connected to. It should automatically enable `MSP` as well.

///_ THIS IS ABOUT WHERE MY KNOWLEDGE WITH DIGITAL ENDS. SUGGESTIONS WELCOME _///
