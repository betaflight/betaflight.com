...
- [RPM controlled multiple dynamic notch filtering over bi-directional DShot](../archive/Bidirectional-DSHOT-and-RPM-Filter), to markedly improve motor noise suppression, reducing filter delay time
- [Launch Control](../archive/Launch-Control), allows a pilot to hold a specified angle precisely and accurately for the perfect race launch
- [D_min](../archive/D_MIN.md), which dynamically adjusts the PID D parameter according to stick movement
...
- [Integrated yaw](../archive/Integrated-Yaw), an experimental option which mathematically integrates the yaw PID values, potentially simplifying yaw tuning
...
### The following notes apply ONLY to un-integrated yaw configuration! If you're flying [integrated yaw](../archive/Integrated-Yaw), the tuning process is completely different!
