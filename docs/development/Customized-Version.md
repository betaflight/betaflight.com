...
In the main Betaflight repository the config submodule is hydrated at `./src/config`, and each board's file is located in `./src/config/configs/[MANUFACTURER_ID]/[BOARD_NAME]/config.h` — configs are grouped by the manufacturer's four-letter code. Targets are still selected by board name alone (`make configs` followed by `make [BOARD_NAME]`).
